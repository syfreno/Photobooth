...
const PhotoBoothGroup = ({ setCapturedImagesGroup }) => {
  const navigate = useNavigate();
  const videoRef = useRef(null);
  const canvasRef = useRef(null);
  const [capturedImagesGroup, setImages] = useState([]);
  ...
  const startCountdown = () => {
    ...
    const captureSequence = async () => {
        if (photosTaken >= 4) {
            ...
            try {
              setCapturedImagesGroup([...newCapturedImages]);
                ...
                console.error("Error navigating to preview group:", error);
            }
            return;
        }
  ...
  return (
    ...
    <div className="photo-container-group">
      <div className="camera-container-group">
        <video ref={videoRef} autoPlay className="video-feed-group" style={{ filter }} />
        <canvas ref={canvasRef} className="hidden" />
      </div>

        <div className="preview-side-group">
          {capturedImagesGroup.map((image, index) => (
            <img key={index} src={image} alt={`Captured ${index + 1}`} className="side-preview-group" />
          ))}
        </div>
      </div>
